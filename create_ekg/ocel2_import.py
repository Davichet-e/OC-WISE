...
class OcelImport:
    ...
    def readJsonOcel(self, dataset:str):
        ...
        self.dataset_baseName = dataset[:-len(".jsonocel")]

        # read zip
        with open(dataset, encoding="utf-8") as jsonOcel:
            # read JSON
            data = jsonOcel.read()  
            d = json.loads(data)    
    ...
    def import_nodes(self, csv, node_label):
        ...
        full_path = os.path.realpath(csv).replace("\\", "/")
    ...
    def ocel2_import_e2o_relation(self):
        ...
        # need full path to csv file for correct import query for neo4j
        full_path = os.path.realpath(self.csv_relations_e2o).replace("\\", "/")
